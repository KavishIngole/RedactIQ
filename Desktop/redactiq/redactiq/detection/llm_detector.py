...
class LLMDetector:
    ...
    def detect(self, text: str) -> list[PIIEntity]:
        ...
        Sends the text through the model with a structured prompt,
        then parses the JSON output into PIIEntity objects.
        """
        if not self.enabled:
            return []
    ...
    def detect_with_api(self, text: str, api_url: str = "http://localhost:8000") -> list[PIIEntity]:
        """Detect PII using a remote vLLM OpenAI-compatible API."""
        prompt = self._build_prompt(text)
